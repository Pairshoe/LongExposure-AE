...
from exposer.utils.profile_utils import traced_mlp_inputs, traced_mlp_activations
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.trace_mlp_inputs = config.trace_mlp_inputs
        self.trace_mlp_activations = config.trace_mlp_activations
    ...
    def forward(
        ...
        if self.trace_mlp_inputs:
            traced_mlp_inputs.append(hidden_states.detach().cpu())
        ...
        if self.trace_mlp_activations:
            _, _hidden_size = hidden_states.shape
            _hidden_states_sum = hidden_states.view(1, -1, _hidden_size).sum(dim=1)
            traced_mlp_activations.append(_hidden_states_sum.detach().cpu())

        # if self.trace_mlp_activations:
        #     traced_mlp_activations.append(hidden_states.detach().cpu())
