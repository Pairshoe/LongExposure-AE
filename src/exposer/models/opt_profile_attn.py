...
from exposer.utils.profile_utils import traced_attn_inputs, traced_attn_scores
...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.trace_attn_scores = config.trace_attn_scores
        self.trace_attn_inputs = config.trace_attn_inputs
    ...
    def forward(
        ...
        if self.trace_attn_inputs:
            traced_attn_inputs.append(hidden_states.detach().cpu())
        ...
        if self.trace_attn_scores:
            # sum over all batches
            attn_weights_sum = attn_weights.view(bsz, self.num_heads, tgt_len, src_len).sum(dim=0)
            traced_attn_scores.append(attn_weights_sum.view(1, self.num_heads, tgt_len, src_len).detach().cpu())
        # if self.trace_attn_scores:
        #     traced_attn_scores.append(attn_weights.view(bsz, self.num_heads, tgt_len, src_len).detach().cpu())
...
OPT_ATTENTION_CLASSES = { "eager": OPTAttention }
